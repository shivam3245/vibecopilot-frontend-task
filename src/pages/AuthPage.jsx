                ...
                <AnimatePresence mode="wait">
                    {isLogin ? (
                        ...
                        >
                            ...
                            <button onClick={handleSubmit} className="w-full py-3 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition-all text-sm md:text-lg">Login</button>
                            <p className="text-black text-center">
                                New user? <span onClick={() => setIsLogin(false)} className="text-blue-600 hover:underline font-bold transition-all cursor-pointer">Sign up</span>
                            </p>
                        </motion.div>
                    ) : (
                        <motion.div
                        ...
                        >
                            ...
                            <input type="password" placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
                                className="w-full p-3 rounded-lg bg-gray-300 bg-opacity-20 text-black placeholder-black outline-none focus:ring-2 focus:ring-green-300 transition-all text-sm md:text-base" />
                            {error && <p className="text-red-400 text-center">{error}</p>}
                            <button onClick={handleSubmit} className="w-full py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-800 transition-all text-sm md:text-lg">Register</button>
                            <p className="text-black text-center">
                                Already have an account? <span onClick={() => setIsLogin(true)} className="text-green-600 hover:underline font-bold md:text-lg transition-all cursor-pointer">Login</span>
                            </p>
                        </motion.div>
                    )}
                </AnimatePresence>
